...
cursor.executemany("""
INSERT INTO Matches (match_id, date, opponent_team, venue, mavericks_score, opponent_score, result)
VALUES (?, ?, ?, ?, ?, ?, ?)
""", sample_data)

conn.commit()

print("1. All matches data:")
...
cursor.execute("SELECT AVG(mavericks_score) FROM Matches")
print("Average Score:", round(cursor.fetchone()[0], 2))

print("\n6. Win percentage:")
cursor.execute("SELECT 100.0 * SUM(CASE WHEN result = 'Win' THEN 1 ELSE 0 END) / COUNT(*) FROM Matches")
